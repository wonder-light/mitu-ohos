 ...
 *
 * Based on AbstractDaoSession.java written by
...
export class AbstractDaoSession {
  ...
  public async insert<T>(entity: T): Promise<number> {
    let dao: AbstractDao<T, any> = <AbstractDao<T, any>> this.getDao(DbUtils.getEntityClassName(entity.constructor));
  ...
  public deleteAll<T>(entityClass: T): void {
    let dao: AbstractDao<T, any> = <AbstractDao<T, any>> this.getDao(DbUtils.getEntityClassName(entityClass.constructor));
    dao.deleteAll();
  }
  ...
  public async load<T, K>(entityClass: T, key: K): Promise<T> {
    let dao: AbstractDao<T, K> = <AbstractDao<T, K>> this.getDao(DbUtils.getEntityClassName(entityClass.constructor));
